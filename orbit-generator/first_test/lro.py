...
spice.furnsh('../kernels/SPK/lrorg_2025166_2025258_v01.bsp') 

# --- 2. BOUCLE CONFIGURATION ---
pas_en_secondes = 60
nombre_de_points = 1000 

# --- 3. LRO SETUP (detection of the ID and dates) ---
ids = spice.stypes.SPICEINT_CELL(100)
spice.spkobj('../kernels/SPK/lrorg_2025166_2025258_v01.bsp', ids)
id_lro = ids[0]

# Get start/end dates from the file
cover = spice.stypes.SPICEDOUBLE_CELL(2000)
spice.spkcov('../kernels/SPK/lrorg_2025166_2025258_v01.bsp', id_lro, cover)
lro_et_debut, lro_et_fin = spice.wnfetd(cover, 0)

# We define the availability interval for the CZML header
# CZML requires ISO 8601 format (e.g., 2012-03-15T10:00:00Z)
iso_start = spice.timout(lro_et_debut, "YYYY-MM-DDTHR:MN:SC.###Z")
# We calculate the end based on the number of points we simulate
et_final_simu = lro_et_debut + (nombre_de_points * pas_en_secondes)
iso_end = spice.timout(et_final_simu, "YYYY-MM-DDTHR:MN:SC.###Z")

print(f"--- Calculating Trajectory for LRO ---")
...
for i in range(nombre_de_points):
    ...
    et_actuel = lro_et_debut + temps_ecoule
    
    try:
        # Calculate position relative to MOON in J2000 frame (Inertial)
        pos, _ = spice.spkpos(str(id_lro), et_actuel, 'J2000', 'NONE', 'MOON')    
...
satellite_packet = {
    "id": "LRO_Sat",
    "name": "LRO",
    "description": "Trajectory of the LRO mission around the Moon.",
    "availability": f"{iso_start}/{iso_end}",
    "billboard": {
        "show": True,
        "pixelSize": 10,
        "color": {"rgba": [180, 0, 255, 255]}, # Violet
    ...
    "label": {
        ...
        "fillColor": {"rgba": [180, 0, 255, 255]},
        "outlineColor": {"rgba": [180, 0, 255, 255]},
        "outlineWidth": 2,
